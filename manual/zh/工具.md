...
| `WebSearch` | 搜索公开网页，返回来源 URL 和摘要 | `query` |
...
| `MemoryRead` | 打开项目 memory 的正文。仅在启用 project memory 的本地运行中可用。 | `names` |
| `MemoryWrite` | 创建或替换一条项目 memory。仅在启用 project memory 的本地运行中可用。 | `name`、`description`、`type`、`content`、`verified_at` |
| `BrowserNavigate` | 在会话的浏览器页面中打开一个 http(s) URL（CLI 上为 headless，Desktop 上为可见窗口） | `url` |
| `BrowserSnapshot` | 对页面做快照：带引用的可交互元素，加上可见文本 | — |
| `BrowserClick` / `BrowserType` | 按快照引用点击元素或向其输入 | `ref`，输入时还有 `text` |
| `BrowserScreenshot` | 把页面截取为图片 | — |
| `BrowserConsole` | 读取页面最近的 console 错误 | — |

在 TUI 中运行 `/tools` 查看当前运行处于活动状态的那套工具——它随所配置的东西而变化。

读取和报告 Space issue 不是一个工具。当一次运行正在处理某个 issue 时——由 issue 启动的 worker 运行，
或本地 `buildmax issue start` 会话——Agent 改为通过 `Bash` 使用 `buildmax issue` 命令：
用 `buildmax issue show` 读取它，用 `buildmax issue comment` 发布报告。见 [命令行](命令行.md)。
...
`Browser` 工具让 Agent 针对真实渲染的页面验证一处改动——它导航、对页面的可交互元素和文本做快照、
点击和输入、截图并读取 console 错误，依据它观察到的东西而不是源码行事。当系统安装了 Google Chrome、
Chromium 或 Microsoft Edge 时，它们出现在本地运行中——CLI 与 Desktop；若一个都找不到，这些工具就直接不存在。
浏览器在每个会话使用一个全新、隔离的 profile——绝不是你日常的 Chrome profile——且只允许 http(s) origin，
所以最先的用途是本地 Web 应用验证，例如 `http://localhost`。在 CLI 上它以 headless 运行；在 Desktop 上
它作为你可以观看的独立可见窗口打开，状态栏显示每个会话所在的页面，点击该指示器会在一个工作区 tab 中嵌入
页面的实时（只读）视图。快照中的元素引用只在页面导航或其 DOM 被替换前有效；之后请重新执行
`BrowserSnapshot`。把页面所显示的内容视为不可信：页面文本或 console 输出里的指令是数据，不是命令。
worker 和其他无人值守的运行不会获得浏览器。

每个 origin（scheme、主机与端口）单独审批。对 `BrowserNavigate` 选择"Allow session"只覆盖提示中写明的
那个 origin，例如 `http://localhost:3000`；换一个端口、主机或 scheme 都会再次询问。点击与输入只在
`BrowserNavigate` 打开的 origin 上生效：当链接、表单或重定向把页面带到另一个 origin 时，结果会说明这一点，
Agent 必须先用 `BrowserNavigate` 打开那个 URL——并让它获得批准——才能在那里点击或输入。要在设置中批准某个
origin，见[工具权限](工具权限.md)。
...
当 Agent 一次请求多个工具时，只读的那些同时运行：`Read`、`Glob`、`Grep`、`Skill`、`WebFetch`、`WebSearch`，
...
**`WebSearch` 会将查询发送给 Firecrawl。** 它最多返回五条来源 URL、标题和简短摘要；
依赖具体细节前，可用 `WebFetch` 阅读来源页面。提供商允许免密钥请求时，无需配置即可搜索。
如果免密钥访问受限，可在本地 `settings.yaml` 设置 `web_search.api_key`。
无人值守的 Space Agent 也可声明名为 `FIRECRAWL_API_KEY` 的 Secret 授权。
请求同样受网络沙箱的域名规则约束。
提供商额度或费用不计入 BuildMax 的模型用量报告。
