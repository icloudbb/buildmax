...
| **Desktop** | Wails 应用，提供未签名的发布下载或源码构建 | 同样的本地能力，配以更丰富的 UI |
...
| **Workflow** | 由依赖关系连接 Agent 步骤的可复用图；相互独立的步骤可以并行运行。生命周期：`draft`、`published`、`archived`。 |
| **Task / TaskRun** | 持久的 Agent 线程及其中的一轮运行或一次尝试。直接创建的 Task 有 Portal 详情页；一个 Task 可以有多个 TaskRun。 |
...
可选的：TaskRun 结果本身仍然是完整且可检查的。Agent 可以直接启动 Task；
Portal 的 Task 详情页会流式展示输出，并允许 Continue 该线程或 Retry 最近一次运行。
