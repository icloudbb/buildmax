...
### 数据库 schema —— 仅向前，Alpha 不保证兼容

模式仅向前推进，没有降级迁移。Alpha 不承诺与旧二进制兼容，也不保证保留旧存储形状。
同一版本可以删除或重命名字段。已有迁移删除明文模型凭证及旧 Issue assignee 列；
重新部署旧二进制不会恢复它们。

升级前阅读该版本声明的起始模式和恢复流程。只对明确测试过的版本组合支持二进制回滚。
其他情况应全新部署，或用匹配的二进制恢复协调备份的数据库与存储桶。
启动时的未知迁移警告只是诊断信息，不是兼容性检查或保证。
