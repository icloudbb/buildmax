...
func newReconcileEnv(t *testing.T) *reconcileEnv {
	...
	definition := fmt.Sprintf(
		`{"schema_version":1,"nodes":[{"id":"collect","type":"agent_task","target_agent_id":%q,"prompt":"collect data"},{"id":"summarize","type":"agent_task","needs":["collect"],"target_agent_id":%q,"prompt":"summarize"}]}`,
		agentA.ID, agentB.ID)
