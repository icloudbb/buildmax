...
损失。发布准备会触发 `./make compose upgrade-drill`：它在 Compose 中把已发布的前序版本镜像升级到
候选版本，再次启动旧镜像，并通过配对恢复完成恢复。在候选版本上演练数据库和存储桶配对恢复、
升级以及凭证轮换仍待验证。共享 Redis 协调已经实现，包括消息历史写入对分布式
...
本地 CLI 新增实验性插件声明式应用连接器：浏览器 OAuth 与 PKCE、默认系统凭证
存储、固定的 GET/读取与 POST/写入操作、令牌刷新，以及写入时的交互式终端确认。
Gmail 样例声明两项读取和一项草稿写入。这不是 Desktop 的连接界面，也不是按
Agent 运行授权；未隔离的本地 Bash 可绕过 CLI。参见[应用连接](../../manual/zh/应用连接.md)
与[委托提案](proposals/agent-app-delegation.md)。

实验性的 `connect mcp` CLI 现在可检查远端 HTTP/SSE 服务器并写入用户级
`mcp.json`。`buildmax mcp` 通过现有 MCP 运行时列出服务器、发现工具 schema
并调用工具。静态 Bearer token 可从指定的环境变量读取；MCP OAuth 与持久 CLI
会话尚未实现。未被服务器标注为只读的调用需要交互式确认。参见
[MCP 服务器](../../manual/zh/MCP服务器.md)。
...
本地 CLI 与 Desktop 具备浏览器能力：当机器上装有系统 Chrome/Edge 时，`Browser`
工具（导航、快照、点击、输入、截图、控制台）驱动一个 Go 自持、按会话隔离的
浏览器，让 Agent 通过 CDP 针对真实渲染页面做验证。导航仅限 http(s)；
`BrowserNavigate` 的会话授权只覆盖一个来源，点击与输入只作用于 `BrowserNavigate`
打开的来源；每个会话的 Profile 隔离；worker 及其他无人值守运行不启用。CLI 以 headless 运行；Desktop 以
...
写入同一数据集，备份数据库、`server-data` 卷和 `server.yaml`，然后通过候选版本的 API 检查数据与一次
新运行，并让来源镜像连接已升级的数据库启动，最后在来源版本下恢复备份。只有当候选版本记录了来源版本
不认识的迁移、且来源版本为 0.2.0-alpha.16 或更新时，来源版本才必须拒绝启动。从 0.2.0-alpha.15 到当前
`main` 台账一致，因此来源版本会启动；还没有任何一次演练观察到真实的拒绝。该演练是在一次性栈上的演练，
不是候选证据。
...
接收运行的 Issue——`required` 还会拒绝没有 Issue 的运行。发布会把每个节点的 Agent 固定到
...
对话内容仍可阅读。
Workflow 详情通过显式的「存为草稿」「发布」「归档」操作驱动生命周期，而非状态控件；版本历史从头部按钮打开，不显示不透明的 workflow id。其定义编辑器是可视化图形画布（React Flow）与 raw JSON 视图并列：步骤是节点，`needs` 依赖是节点间连线，选中节点的 agent、指令、Issue 访问与输入绑定在检视面板中编辑，因此分叉 DAG 不再需要手写 JSON。`input_schema`、`result` 与节点的 `output_schema` 经可视化编辑器无损保留、由 raw JSON 编写。
Agent 详情的运行、保存和删除操作也采用同一层级；标签栏支持方向键和窄屏横向滚动。
Agent 的 schedule 在已触发 Task 加载失败时展示错误并可就地重试。
Artifacts 的上传、下载、分享和删除也使用共享操作层级。列表成功加载后才显示数量；
制品名称是链接，预览加载失败时可就地重试。
Issue 详情中的子 Issue、运行、讨论与结果操作，以及 Administration、Space 设置、Files、
Marketplace、登录页和共享的不可用提示、复制、修订控件，现在都采用同一操作层级；
...
状态更新，因此过期卡片会被拒绝，也不会启动任何运行。Board 没有拖拽、排序值或另存的列状态。组件样张现服务于认证外壳之外的 `/specimen`：它一次性呈现共享操作语法、语义 token、状态词汇与全部资源状态，覆盖明暗两主题，并复用真实组件。非作者操作员对工作旅程的评审尚未完成。
...
/readyz 的对象存储检查失败，并在桶完好的情况下恢复）、worker 对象存储写入拒绝（只拒绝
worker 的写入且 /readyz 保持健康时，首次 Run 在种子检查点处失败，Continue 在运行状态处失败，
并保留其回复和一个可下载的 Artifact，且不记录指向不存在对象的 trace 指针）和 Bash 隔离探针。调度器单元测试覆盖
