...
> **受众：** 贡献者、产品评审者与运维人员 · **状态：** 部分实现——已接受的自适应图方向仍在计划中，持久化图运行时正在分片落地。带保护的 compare-and-set Run/节点转换、原子停止意图、幂等 Task 准入、协调租约、图协调器，以及由 Server 持有的到期 Run 恢复循环均已实现。`Service.Reconcile` 从持久状态中折叠已完成的节点、派发就绪节点并安排该 Run 的下次协调；启动时和周期性扫描能够从回调丢失或 Server 重启中恢复。定义现在带有显式的 `schema_version: 1`，并可声明 `input_schema` 与 `result` 选择器，两者均在发布期校验。启动运行时会对照该 `input_schema` 校验调用方输入并冻结到 run 上，且 Portal 依据该模式生成输入表单。每一条按步的记录都是 `WorkflowNodeRun`，会持久化其节点收到的完整已解析输入，以及被接受的 TaskRun 产出的完整输出。节点绑定通过 `source`（`workflow.input` 或某个前驱节点的输出信封：文本、结构化输出、Artifact 引用）加一个 RFC 6901 `pointer` 选取值，且定义可声明一个 `result` 选择器，运行成功时把其取到的值存到 run 上并在 run 及其 issue 上展示。定义现在是一张图：由 DAG `needs` 连接的 `nodes` 决定执行顺序（数组位置不再是权威），发布期校验无环、边存在与仅引用前驱的绑定。每一次协调都会分发所有就绪节点，受 `policy.max_parallel_nodes` 与部署上限约束，因此相互独立的分支可以并行；失败为 fail-fast，并会在结束运行前排空与失败节点并行运行的兄弟节点。图引擎中仍待实现的是：类型化路由、运行期受 schema 约束的路由与自适应控制
...
但这还不是本文档要设计的那个完整运行时，不过它的执行平面已经持久，且定义如今已是一张图。定义是由 DAG `needs` 边连接的一组 `nodes`：一个节点在它所需的每个节点都成功后才就绪，因此由依赖关系——而非数组位置——决定执行顺序；发布期会拒绝有环的图、指向缺失节点的 `needs` 边，或读取非传递前驱节点的绑定。节点可以声明 `output_schema`；共享运行时会校验最终值，并把它持久化到 TaskRun 与节点运行。节点还可以把从 run 输入或某个前驱节点的输出信封（文本、结构化输出或 Artifact 引用）里、按 RFC 6901 指针选取的值绑定到自己的输入中，作为带标签的不可信数据，它读取的是上游节点运行在成功时持久化下来的完整输出，以及被接受的 TaskRun 归属的 Artifact。推进是一次基于持久事实的协调：Task 准入在一个稳定键下是幂等的，一个有边界的租约减少重复的协调轮次，带保护的 compare-and-set 转换是正确性机制，而一个由 Server 拥有的恢复循环会去收尾那些因回调丢失或重启而搁浅的运行。每一次协调都会折叠所有已完成节点并分发所有就绪节点，受 `policy.max_parallel_nodes` 与部署上限约束，因此相互独立的分支可以并行；失败为 fail-fast，并会在结束运行前排空与失败节点并行运行的兄弟节点。相对目标而言仍然缺失的是图引擎的其余部分：类型化路由与等待、运行期受 schema 约束的路由，以及自适应控制。
...
本记录接受的是这套架构本身，而不是要求立即扩大范围。可靠性这个根基，必须先于新的 Workflow 控制流之前建成。静态图执行作为一项 R5 能力，已在这一根基之上交付（§1.1）。动态扩展、人工等待和入站事件仍然留待更晚的阶段，要等到 R0–R3 的 Beta 证据与后续产品证据证明其必要性后才会被选中；只有具体部署提出需求和优先级时才提前。周期性运行已在 Task 平面上交付：一个 schedule 触发一个 Agent 或一个 Workflow（参见[定时 Agent 执行 §15](定时Agent执行.md)）。

本记录首先建设的持久协调 substrate——幂等的 Task 准入、协调、基于租约的恢复，以及 compare-and-set 状态转换——是刻意做到与形态无关的。线性运行或静态图运行所需的这套 substrate，与一次有界的 Agent 间委派所需的完全相同：委派同样需要准入持久的子 Task、在不占用 worker 的情况下等待、并在回调丢失后恢复父任务（参见[助手编排与 Workflow 边界](../proposals/assistant-orchestration-and-workflow-boundary.md)）。因此先建它是一笔无悔投入，不会把产品绑死在图的广度上：无论 Workflow 是在静态 DAG 之外继续拓宽，还是收窄为围绕自适应委派的确定性 Automation 外壳，这一层都原样复用。Workflow 已在其上拓宽为静态 DAG；还要交付多少更多的图的广度，被推迟到更晚的证据出现时再定；而 substrate 当初不必等。
...
Workflow 详情页面按生命周期状态来组织这些组件，而不是一次性把所有操作入口都摊开，因为 `draft` 与 `published` 的 Workflow 被用于不同的目的。处于 `draft`（或 `archived`）的 Workflow 以编写布局打开：主操作是 Publish，主体是编辑表单和只读拓扑视图，并且不出现手动 Run——反正运行时也会拒绝运行未发布的定义。处于 `published` 的 Workflow 以运行布局打开：主操作是 Run，编辑退到一个显式的 Edit 入口之后，主体以只读拓扑视图和最近的运行记录打头，而不是一个可编辑的表单，这样阅读者无需进入编辑界面就能运行或查看。无法管理 Workflow 的成员看到的是没有 Edit 入口的运行布局。运行输入（第 4 项所述、由 `input_schema` 生成的输入）在由 Run 操作打开的 Run 抽屉里收集，而不是只要存在 schema 就内联渲染。修订版本历史是按需从页头进入的次级信息，而不是常驻的一栏；恢复操作仍在那里可用。只读拓扑视图（第 3 项）复用运行详情页面的 DAG 渲染；直接的画布操作（第 6 项）仍然是独立的、以证据为前提的后续项，不在本次组织的范围内。
...
- 用 `needs` 替换掉“数组位置即执行权威”的做法。**已交付：** 定义是一组带 DAG
  `needs` 的 `nodes`；节点运行会快照其依赖边；就绪由这些边决定；发布期校验无环、边存在
  与仅引用前驱的绑定；一次运行按确定性拓扑序记录每个节点的 index。
- 在并发限制之内，调度所有已就绪的节点。**已交付：** 协调器每一趟都会折叠所有已完成节点
  并分发所有就绪节点，受 `policy.max_parallel_nodes` 与部署上限 `MaxParallelNodesCeiling`
  约束。
- 实现确定性的扇出/扇入,以及快速失败式的阻塞语义。**已交付：** 扇出会一起分发其就绪的
  依赖节点，扇入会等待其全部前驱，节点失败会阻塞所有待执行节点，通过 Task 取消协议停止
  并行运行的兄弟节点，并且只在每个已接纳 TaskRun 都进入终态后结束运行。中间的 `failing`
  与 `canceling` 状态会跨 Server 重启保留。
- 加入完整的发布校验,以及一个只读的图视图。**已交付：** 发布校验，包括把每个节点的 Agent
  固定到某一 revision（未指定的固定到当前 revision），使运行永不解析“latest”；以及一个按
  依赖布局的只读运行图，每个节点按状态着色并链接到其 Task。
- 把线性形式保留下来，作为最简单的一种 DAG，而不是一个独立的引擎。**已交付：** Portal
  的步骤表单编写一条线性链，序列化为 `nodes`/`needs`。
