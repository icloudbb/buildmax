...
并实测恢复时间。kind 演练不算候选版本证据。发布准备会触发 Compose 升级演练：它演练前序版本镜像
升级到候选版本，随后让旧镜像连接已升级的数据库启动，并通过恢复升级前的备份完成恢复。这同样只是
演练，不是候选证据。尚无候选版本验证过数据库与存储桶配对恢复、自身的升级或凭据轮换。

**下一步：** 剩余的生命周期证据——在候选版本自己的依赖上完成数据库与存储桶配对恢复、从前序版本二进制
出发的升级，以及候选版本上的凭据轮换——其中数项会作为 R3 的运维旅程落地。
...
类型化数据绑定已实现。失败和步骤取消现在会在 Workflow 结束前排空实际 TaskRun，
并具备原子 Task 准入/节点链接以及停止意图的重启恢复。候选工作包括条件路由、更多聊天平台适配器、可执行 Space 插件、
