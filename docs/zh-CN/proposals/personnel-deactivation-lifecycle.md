...
整个契约建立在两个性质不同的权限轴之上，把它们区分清楚正是恢复可预测的原因：

- **部署级账户停用是盖在被保留状态之上的可逆闸门。** 成员记录、Space 数据和
  webhook key 全部保留，只是在 `disabled_at` 生效期间被拒绝。重新启用即重开闸门，
  无需任何"复活"步骤:成员记录从未删除，因此立刻重新生效。已停用账户继续占用其
  唯一邮箱:`email` 始终是唯一键,因此第一个账户存在期间,该邮箱不能被注册到第二个
  账户。同一个人通过重新启用该账户回归,而不是新建账户;邮箱也绝不会改绑给另一个人。
  释放某个邮箱需要删除账户,而这仍是非目标(§13)。
- **单个 Space 成员移除是对单段关系的破坏性变更。** `space_member` 记录被删除，
  因此之后的重新邀请是一次带新 `created_at` 的全新加入，而不是对旧关系的恢复。

执行资格是两个事实的 AND:账户未停用**且**成员记录存在。应把停用视为可逆的暂停、
把移除视为"先删再新建"的关系,不要期望二者对称。
...
`disabled_at` 为闸门。这一点已经裁定，而非悬而未决:临时暂停**保留**已被拒绝的
webhook key，以便显式重新启用时原样恢复集成;有引导离职路径则**永久退役**这些 key。
默认退役，因为必须比个人存在更久的集成需要单独设计的机器主体，而不是被遗忘的个人
凭证。
...
移除仍是对单条 `space_member` 记录的硬删除，与当前存储一致。离职的来源信息保存在
成对的 `space.member_removed`/`space.member_added` 审计事件中，而不是保留行或软删除
行,因此返回的成员是一条带新 `created_at` 的新记录,符合显式且不复活的恢复语义
(不变量 6)。不新增软删除列:本契约不需要它,而保留行还会与 `space_member` 在
`(space_id, user_id)` 上的唯一索引冲突。
...
第二个验收场景对称地检验 Space 轴,与重新启用路径相对应。一位在某共享 Space 拥有
一个启用 Schedule、一个 PENDING run 和一个 RUNNING run 的成员被移出该 Space。测试
必须展示该 Space 的派生工作以 `creator_not_member` 原因暂停或取消、同一人在另一个
Space 的工作与访问不受影响、Space 所有的结果对剩余成员仍可读,且之后的重新邀请生成
一条带新 `created_at` 的全新成员记录,不复活任何已暂停 Schedule 或已取消 run。
...
2. 仅限 owner 已停用时由 System Administrator 恢复是否可接受，还是每个部署都必须在
   离职前准备第二位 Space owner？
3. 目标部署是否需要托管 CLI/Desktop SSO，从而把原生 Session 和本地凭证清理纳入同一
   journey？
4. 在何种实际规模下，执行资格 reconciler 才需要持久工作队列，而非有界 SQL scan？
