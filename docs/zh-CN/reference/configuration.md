...
在无法产生任何内容的轮次上，该调用会被跳过，因此只有在很可能有内容可展示时才花费这些额外的 token。回顾摘要讲的是“改动了什么”，所以只有真正改变了状态的轮次——写了文件、运行了命令——才会生成，而且仅当回复没有已经把该改动详细讲清时才生成；只是作答、只读或只搜索的轮次不会生成回顾摘要，因为回复本身就是它的完整交代。只有以向你提问结束的轮次才会给出建议。它所花费的部分会计入该会话的用量——TUI 中的 `/info`，Desktop 中的状态栏。将任意一个键设为 `false` 即可单独关闭对应的一半功能，两者都设为 `false` 则该轮结束时完全不会产生额外调用。
