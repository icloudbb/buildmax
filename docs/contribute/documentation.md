...
[`docs/README.md`](../README.md) is the task-oriented landing page. There is no
separate exhaustive file index: a single page listing every document had to be
edited by almost every documentation change, so it conflicted constantly under
parallel work. Navigate through the per-directory `README.md` files instead.
