...
| [Agent runtime introspection and context control](agent-runtime-introspection-and-control.md) | Agent Runtime and Models | Should an Agent observe its own live run, context, budget, and Todo state, and may it request compaction without owning history integrity? | Existing events, statistics, durable Todos, user-facing status, and compaction are foundations; no LLM-facing self-inspection or compaction-request tool exists |
