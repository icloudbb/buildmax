...
| [Agent browser capability](browser-capability.md) | Local Experience | Should the runtime own a Go-driven browser that lets an Agent verify against a real rendered page, enabled first on local surfaces, and what evidence would justify embedding it in the Desktop workspace? | Research only; no BuildMax browser prototype or implementation |
...
| [Agent delegation to user applications](agent-app-delegation.md) | Local Experience and Trust | Can a workspace-centered Agent become a useful entry point to connected applications while keeping authorization and approval understandable? | Plugin connector CLI with OAuth and a Gmail sample; remote MCP connection and call CLI; no per-run grants or hardened Agent boundary |
